...
const BotName = 'ESTER - BOT Dark 2k15'; 
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Dark 2k15\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521982310081:5521982310081\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#','_'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ Espere fofo,lindo e gostoso',
					success: '️❬ ✔ ❭ Sucesso feio(a) 🖤',
				...
				only: {
					...
					ownerG: '[❗] Este é um recurso especial para o DARK 2K15❌',
					ownerB: '[❗] Este é um recurso especial para o Dark 2k15 ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Dark gostoso. Registre-se para fazer amizade com o bot Dark gostoso por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Dark|15\n\n──「 ESTER BOT 」──`,
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (isGroupAdmins) return reply('se tu não fosse admin, eu te bania lixo, rlx 🙂')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("menos de 1 segundo KKKKKKKK tchau otário e chupe meu ovo no pv 😔🤙")
		}, 0)
	}
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("1 segundo KKKKKKKK tchau otário, c de cuzaum 😔🤙")
		}, 0)
	}
	...
	if (messagesC.includes("fdp")){
        ...
        if (messagesC.includes("Jaine")){
			client.updatePresence(from, Presence.composing)
			reply("Enquanto os meus olhos vêem como você é linda por fora. O meu coração sente o quanto você é perfeita por dentro🌻.")
			...
			switch(command) {
				...
				case 'ramaljadian':
                   ...
                   case 'gcpf':
                    ...
                    break
				case 'ytmp4':
					if (args.length < 1) return reply('Cadê o link, kct?')
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modoanime':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('ta errado caralho')
					if ((args[0]) === 'on') {
...
// recursos anime, random @Dark 2k15 domina irmão
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "Dark 2k15", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o dark gostoso ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN BOT ESTER - Dark 2k15 🇧🇷* 」\n*│+ wa.me/21982310081*╰──────*「 *Dark 2k15* 」*────*\n\n*_SE QUER SER ADMIN DO BOT ESTER_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*puta'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Dark 2k15\n*WPP:* wa.me/+5521982310081\n*INSTA:* sai\n\n\nEspero que tenham gostado do bot 🐒🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'admins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('{ ❗ } *Marque a figurinha animal*')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
